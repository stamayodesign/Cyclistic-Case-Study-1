...
```{r Year 2022 Clean, First 10 rows, echo=FALSE}
...
kable(year_2022_bike_type_Casual_df,caption = "Number of Trips per Day of the Week, 2022 - Casual") %>% 
  kable_styling()
```
...
3. Additional tables to be used by functions (Not displayed for convenient, but found in the [Appendix here](#Appx2))

### <a name="Ref3" />Trends and Relationships

The next step was to find trends in the data. 

#### <a name="Ref2" />Trip Frequency
...
```{r View graph_year2022_tripfreq_customer}
graph_year2022_tripfreq_customer
```

As you can see, the number of trips for members peaked in August, while the number of trips for casual customers peaked a month earlier in July. And while the May to July shows that there is a relatively equal amount of trips between casual customers and members, the rest of the year has far more members using the service compared to casual customers.

#### <a name="VizRef1" /><a name="Ref4" />Bike Preferences 
...
```{r Year 2022 Bike Popularity Casual}
graph_year2022casual_numOfBike_month_biketype_sansDocked
graph_year2022casual_numOfBike_month_biketype
```
You can click between the two preview graphs to see the data without the docked bikes.

From these graphs, we can see that, aside from the month of May and June, casual rider use more electric bikes compared to classic. Furthermore, from July to December, there is a significant increase of electric bikes being used over classic bikes.
...
<a name="VizRef2" />
```{r Year 2022 Bike Preferences Member }
graph_year2022member_numOfBike_month_biketype_sansDocked
```
From this graph, we can see that, there are more classic bikes used compared to electric in from January to August, but then electric bikes are used more often from September to December.

This graph does not feature docked bikes due to members not using docked bikes. To see a deeper dive into this data, you can [click here](#MoreViz2) to view each bike with number labels.
...
### <a name="#MoreViz1" /> Year 2022 Bike Popularity, Casual Interactive
[Jump to report location this is used](#VizRef1)
```{r figures-side Casual, fig.show="hold", out.width="20%",fig.align='center'}
...
```
### <a name="#MoreViz2" /> Year 2022 Bike Popularity, Member Interactive
[Jump to report location this is used](#VizRef2)
```{r figures-side Member, fig.show="hold", out.width="20%",fig.align='center'}
...
1. Casual customers use bikes more frequently in the hot summer months. **Create summer promotions to help convert casual customers to members.**
2. Casual customers use more electric bikes over classic bikes. **Creation promotions advertising electric-powered bikes. Increase number of electric bikes available.**
3. Casual customers take more time on their trips through the service comparatively to current members. Current members also do not use docked bikes. **Advertising should include member testimonials about how much time is saved by not having to use the docked bikes.**
...
#### <a name="Appx3"/>Year 2022 Customer Trip Frequency
...
#### <a name="Appx4"/>Year 2022 Bike Preferences, Casual and Member
...
```{r Year 2022 Bike Preferences Member Plots, include=TRUE, eval=FALSE}
# Year 2022 Bike Popularity for Member Customers
graph_year2022member_numOfBike_month_biketype <- ggplot(data=yearByMonth_2022_bike_type_Member_subgrouped_df,aes(x=monthNum,y=num_of_bike, fill = biketype)) +
  geom_bar(stat = "identity",width=.9,position=position_dodge(width=1))+
  labs(title = "Year 2022 Bike Popularity, Member",
...
```{r Year 2022 Bike Preferences Member, include=TRUE, eval=FALSE }
graph_year2022member_numOfBike_month_biketype_sansDocked
```

#### <a name="Appx5"/>Trip Length 
